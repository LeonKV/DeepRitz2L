...
filepath_prefix = f"results_singular_solution_k{k_smoothness}/"
...
list_n_epochs = [25 for _ in list_n_per_dim]
...
for idx_n, (n_per_dim, n_epochs) in enumerate(zip(list_n_per_dim, list_n_epochs)):
    ...
    mask = torch.min(array_dist, dim=1).values > 1/10 * (2 * problem.domain.radius / n_per_dim)      # 1/10 extra to not create too large holes
